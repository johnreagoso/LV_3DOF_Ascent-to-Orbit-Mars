...
function check = GA_ATO_EXEC_wPtchDwn(select_no, distro_option_output)
...
%% Main Body Below to Run Multiple GA 300 generation runs
    ...
    while ga_run_count <= 100
        ...
        [J_COST_AVG, J_COST_MAX(ga_run_count), TRAJPOP_INDV_NEW ] = ...
            GA_TRAJ_MAIN_BODY_wPitchDwn(TRAJPOP_INDV, select, distro_option, ga_run_count);
        ...
        % below ensures that if the leading candidate solution has not
        % increased in fitness in over 10 generations, the GA run is
        % terminated:
